...
import com.kinapto.fitadapt.data.local.entity.ArticleEntity
import com.kinapto.fitadapt.data.local.entity.GoalEntity
...
class HomeViewModel @Inject constructor(
    ...
    private fun calculateAdherence() {
        viewModelScope.launch {
            val card = _uiState.value.activeCard ?: return@launch
            val targetPerWeek = card.targetSessionsPerWeek ?: return@launch
            if (targetPerWeek <= 0) return@launch

            val startOfWeek = com.kinapto.fitadapt.util.DateUtils.getStartOfWeek()
            val now = System.currentTimeMillis()
            
            val sessionsThisWeek = sessionRepository.getSessionsInRange(startOfWeek, now).firstOrNull()?.count { it.completed } ?: 0
            val adherence = (sessionsThisWeek.toFloat() / targetPerWeek.toFloat()).coerceIn(0f, 1f)
            
            _uiState.update { it.copy(adherence = adherence) }
        }
    }
    ...
    private fun updateGoalsProgress(goals: List<GoalEntity>) {
        viewModelScope.launch {
            val now = System.currentTimeMillis()
            val startOfWeek = com.kinapto.fitadapt.util.DateUtils.getStartOfWeek()
            val startOfMonth = com.kinapto.fitadapt.util.DateUtils.getStartOfMonth()
            ...
            goals.forEach { goal ->
                ...
                val needsReset = com.kinapto.fitadapt.util.DateUtils.isPeriodExpired(
