...
import com.kinapto.fitadapt.data.repository.ProgressRepository
...
data class ProgressUiState(
    ...
    val targetSessionsPerWeek: Int = 0,
...
class ProgressViewModel @Inject constructor(
    ...
    private val cardRepository: com.kinapto.fitadapt.data.repository.TrainingCardRepository,
    ...
    fun loadProgress() {
        ...
        viewModelScope.launch {
            cardRepository.getActiveCard().collect { card ->
                if (card != null) {
                    val target = card.targetSessionsPerWeek ?: 0
                    if (target > 0) {
                        val startOfWeek = com.kinapto.fitadapt.util.DateUtils.getStartOfWeek()
                        val now = System.currentTimeMillis()
                        val count = sessionRepository.getSessionsInRange(startOfWeek, now).first().count { it.completed }
                        val adh = (count.toFloat() / target.toFloat()).coerceIn(0f, 1f)
                        _uiState.update { 
                            it.copy(
                                weeklyAdherence = adh,
